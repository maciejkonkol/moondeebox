...
abstract class Moondee_Entity extends Moondee_Application_MoondeeDatabaseObject
{
    ...
    */ 
    ...
    /**
    * Metoda sprawdza czy album istnieje. Jeśli istnieje zwraca ten album
    *
    * @param integer | string $album Id lub nazwa albumu
    * @return Moondee_Image_Album
    * @access public    */ 

    public function albumExist( $album ) {
        if( $this->albums ){
			if( is_int( $album ) ){
			...
			return Moondee_Image_Album_Helper::albumExist( $this->id, $album );
		}
    }
    ...
    */ 
    public function addAlbum( $album_name ) {
		...
		$proxy_album = new Moondee_Application_Proxy( $album );

        if( $this->albums ){
            $this->albums[ $proxy_album->getId() ] = $proxy_album;
        }
    ...
    * @access public
    */ 
    public function getImages( $limit = null, $start = null ) {
        return Moondee_Image_Helper::getEntityImages( $this->id, $limit, $start );
    }
